...
layout = html.Div([
    html.H2('ETH/BTC Risk Metric'),
    html.Div(dbc.Spinner(size='lg', color='black', type='border'), id='ethbtc-risk-graph'), 
])

@callback(Output('ethbtc-risk-graph', 'children'),Input('eth-data', 'data'))
def ethbtc_risk(data):
    ...
    df['MA'] = df['Close'].rolling(374, min_periods=1).mean().dropna()
    df['Preavg'] = (np.log(df.Close) - np.log(df['MA'])) * df.index**.395
    ...
    fig.update_yaxes(title='eth / btc price',
